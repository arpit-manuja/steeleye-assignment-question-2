function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
  const startingmarkTag = "<mark>";
  const endingmarkTag = "</mark>";

  let highlightedContenttext = htmlContent;
  ...
  for (const position of plainTextPositions) {
    ...
    const highlightedtextWord = `${startingmarkTag}${plainWord}${endingmarkTag}`;

    highlightedContenttext = highlightedContenttext.replace(
      new RegExp(plainWord, "g"),
      highlightedtextWord
    );
  }

  return highlightedContenttext;
}
